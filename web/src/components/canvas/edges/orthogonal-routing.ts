...
export function dragSegment(args: {
  ...
  if (seg.isSourceAnchored && !seg.isTargetAnchored) {
    ...
    // Offset corner — short distance along the source-handle direction.
    const offset_a: Waypoint = {
      x: source.x + ANCHOR_INSERT_OFFSET * dir.dx,
      y: source.y + ANCHOR_INSERT_OFFSET * dir.dy,
    };
    // Step-up corner — perpendicular jump to the dragged value.
    const offset_b: Waypoint = isH
      ? { x: offset_a.x, y: perpendicularValue }
      : { x: perpendicularValue, y: offset_a.y };
    // The original first waypoint (seg.b) must shift to the new perpendicular value
    // so that the segment user dragged ends up at the right perp coord.
    const updatedFirstWp: Waypoint =
      wpEnd < working.length
        ...
        : isH
          ? { x: target.x, y: perpendicularValue } // shouldn't happen if both anchored case is above
    ...
    const next: Waypoint[] = [
      ...
      offset_b,
      updatedFirstWp,
      ...working.slice(wpEnd + 1),
    ];
    return next;
  }

  // Case C — target-anchored: insert two new corners before the target.
  if (seg.isTargetAnchored && !seg.isSourceAnchored) {
    const dir = targetUnitVector(targetPos);
    // Approach-corner — short distance back from target along the
    // direction the wire COMES FROM. dir is "into target", so "from
    // target back along the segment" is -dir.
    ...
    // The original last waypoint (seg.a) must shift to the new perpendicular.
  ...
  // Case D — both source AND target anchored (single segment, no
  // intermediate waypoints). Insert FOUR corners to materialise both
  // sides. Less common (only happens for source.y == target.y on
  // right/left edges, etc.). For v3.0 keep it simple: bail out and
  // refuse this drag rather than emit a messy 4-corner insertion.
  return working;
}
