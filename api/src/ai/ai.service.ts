...
export class AiService {
  ...
  private systemPrompt(): string {
    return [
      ...
      "- VALIDATION RULES the designer enforces (must follow or your output will be rejected):",
      "  • Every `userTask` needs `data.assignment` with a non-empty value: `{ type: \"directUser\" | \"role\" | \"expression\", value: \"<uuid|roleKey|${path}>\" }`. Prefer role-based assignment unless the user explicitly names a person.",
      "  • Every exclusive/inclusive gateway with conditions on every outgoing flow MUST also have `data.defaultFlowId` set to one of its outgoing edge ids — otherwise an instance with no matching condition will fail.",
      "  • Every `serviceTask` needs `data.implementation` set: `{ type: \"externalWorker\", config: { jobType: \"<topic>\" } }` is the safe default. Other valid types are `\"rest\"` (with `config.url`) and `\"connector\"` (with `config.connectorId` + `config.operation`).",
      "  • Every node must be reachable from a start event via sequence flows — don't leave disconnected sub-flows.",
      "  • FEEL condition syntax: literals, identifiers + member access, arithmetic, comparison (`==`, `!=`, `<`, `>=` etc), `&&`, `||`, ternary. NO `eval`, `Function`, `this`, statements, semicolons, or assignment.",
      "  • Assignee expressions use the strict `${variable.path}` form only.",
